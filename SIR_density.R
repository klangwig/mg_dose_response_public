...
################### Function to run Models ##################
outSIR <- function (par1,par2,params){
  
  ic1 = pgamma(x_s,shape=par1,scale=par2)
  ...
  outtmp$S = rowSums(outtmp[,2:(bins+1)])
  outtmp$D = N-(rowSums(outtmp[,2:(bins+3)]))# deaths
  keeps <- c("S","I","R","C","D")
  outtmp = outtmp[keeps]
  ...
  outtmpH$D = N-(rowSums(outtmpH[,2:4]))# deaths
  keeps <- c("S","I","R","C","D")
...
par.shape1 = input_parms$par1.gamma[input_parms$bird.groups=="750 va"]
par.scale1 = input_parms$par2.gamma[input_parms$bird.groups=="750 va"]

mylist <- list()
for (j in 1:length(par.shape1)){
  shape1 = par.shape1[j]
  scale1 = par.scale1[j]
  
  outtmp1 <- outSIR(shape1,scale1,params)
...
pC <- ggplot(df750, aes(x=Cdiff)) + 
  ...
  labs(title="Low dose (750)", x="Difference in epidemic size", y="Density") +
...
pD <- ggplot(df750, aes(x=Ddiff)) + 
  geom_density(fill="#00BFC4")+
  labs( x="Difference in deaths", y="Density") +
...
par.shape1 = input_parms$par1.gamma[input_parms$bird.groups=="30000 va"]
par.scale1 = input_parms$par2.gamma[input_parms$bird.groups=="30000 va"]
...
ggsave(file="Fig/Density_fittedmortality.png", g1) #saves g
...
ggsave(file="Fig/Density_fixedmortality.png", g2) #saves g2
